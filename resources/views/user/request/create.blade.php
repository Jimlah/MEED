    ...
    <div class="grid w-full h-full grid-cols-2 gap-5">
        <div class="flex flex-col p-5 space-y-2 bg-white md:col-span-1 col-span-full bg-opacity-90" x-data="{ tab: 'foo' }">
            <div class="flex items-start justify-start">
                <button class="p-2 text-sm bg-blue-300 focus:outline-none" :class="{ 'active bg-gray-100': tab === 'foo' }"
                    x-on:click="tab = 'foo'">Self</button>
                @if (auth()->user()->role != 4)
                        ...
                        <form action="{{ route('requests.store') }}" class="" method="POST">
                            ...
                            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                <div class="flex flex-col col-span-1 ">
                                    <label for="firstname" class="text-sm font-bold text-gray-700 uppercase">First
                                        Name</label>
                                    <input type="text" name="firstname" id="firstname"
                                        class="px-3 py-2 border-2 border-[#0797E0] focus:outline-none">
                                </div>
                                <div class="flex flex-col col-span-1">
                                    <label for="lastname" class="text-sm font-bold text-gray-700 uppercase">Last
                                        Name</label>
                                ...
                                <div class="flex flex-col col-span-1 md:col-span-2">
                                    <label for="lastname"
                                        class="text-sm font-bold text-gray-700 uppercase">Description</label>
                                    <textarea type="text" name="description" id="description"
                                        class="px-3 py-2 border-2 border-[#0797E0] focus:outline-none resize-none form-textarea block w-full text-left"
